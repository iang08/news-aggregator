...
def format_brief(result: TriageResult, date_str: str) -> str:
    ...
    # Make a fallback brief visibly distinct — Ian should know at a glance when
    # he's reading the local model's output (slightly lower quality) because
    # Claude was unavailable that morning.
    if engine.startswith("local"):
        lines.append(f"> ⚠️ Generated by **local fallback** ({engine}) — Claude was unavailable this morning.")
        lines.append("")
    lines.append("---")
    lines.append("")
